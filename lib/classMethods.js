...
module.exports = function (sequelize, getRefModel, Grant) {
	 ...
	 *     are not repeated
	 */
	AccessTree.prototype.availableGrants = function* (refId) {
		const result = new Map();

		function addGrants(ref, inherited) {
			//for each grant of this ref
			ref[Grant.options.name.plural].forEach(function (grant) {
				let roles = result.get(grant.UserId);
				...
				if (roles.indexOf(grant.role) === -1) {
					if (inherited) {
						roles.inherited.push(grant.role);
					} else {
						roles.own.push(grant.role);
					}
				}
			});
		}
	...
	/**
	 * Returns array of IDs representing the path to the node from the user root
	 *
	 * @param {number} UserId
	 * @param {number} refId
	 * @param {Array.<string>} [inRoles] the roles needed for a node to be considered a match
	 * @returns {Array.<number>} array of ids or ancestor the user has rights for
	 */
	AccessTree.prototype.userRootPath = function* (UserId, refId, inRoles) {
		const grantWhere = {UserId};
		if (inRoles != null && Array.isArray(inRoles)) {
			grantWhere.role = {$in: inRoles};
		}

		let withAncestorsAndGrants = yield getRefModel().findAll({
			include: [
				{
					model: Grant,
					as: Grant.options.name.plural,
		...
		let userRootIndex = 0;
		//walk while you reach one with grants:
		while (
		withAncestorsAndGrants.length > userRootIndex &&
		withAncestorsAndGrants[userRootIndex][Grant.options.name.plural].length === 0
			) {
			userRootIndex++;
		}
		//not a single reference with rights
		if (userRootIndex === withAncestorsAndGrants.length) {
			return [];
		}

		return _.map(withAncestorsAndGrants, 'id').slice(userRootIndex);
	};
